...
operations_path = Path("assets/css/operations.css")
operations = operations_path.read_text(encoding="utf-8")
inventory_header = r'''.inventory-page-head-v11109{
  position:relative;
  display:grid;
...
.inventory-page-head-v11109>div:first-child{
  width:100%;
  min-width:0;
  max-width:none;
}
.inventory-page-head-v11109 h2{
  margin:0 0 7px;
  color:#173b63;
...
.inventory-page-actions-v11109 .btn{
  min-height:44px;
  padding-inline:15px;
  border-radius:10px;
  font-size:11.5px;
...
@media(max-width:1180px){
  .inventory-page-head-v11109{grid-template-columns:1fr;}
  .inventory-page-actions-v11109{width:100%;max-width:none;justify-content:flex-start;}
}
@media(max-width:720px){
  .inventory-page-head-v11109{padding:18px 16px 16px 19px;row-gap:14px;}
  .inventory-page-head-v11109 p{font-size:13px;line-height:1.52;}
  .inventory-page-actions-v11109{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));}
  .inventory-page-actions-v11109 .btn{width:100%;min-width:0;white-space:normal;}
}
@media(max-width:480px){
  .inventory-page-actions-v11109{grid-template-columns:1fr;}
}

'''
operations = replace_one(
    operations,
    r"\.inventory-page-head-v11109\{.*?(?=\.inventory-source-bar-v11109\{)",
    inventory_header,
    "inventory header owner",
    flags=re.S,
)
operations = operations.replace(
    "CRM Suministros V11.21.0 · Familia CSS canónica",
    "CRM Suministros V11.21.1 · Familia CSS canónica",
    1,
)
...
new_grid = (
    ...
    "gap:14px;padding:16px;background:#f7f9fc}\n"
    "@media(max-width:720px){.guided-action-grid{grid-template-columns:1fr;gap:11px;padding:11px}}\n"
)
marker = ".guided-action-card{"
if marker not in core:
    raise SystemExit("guided action card marker missing")
core = core.replace(marker, new_grid + marker, 1)

core = replace_one(
    core,
    r"\.guided-action-card\{[^{}]*\}",
    ".guided-action-card{--action-accent:#1479df;position:relative;min-width:0;min-height:132px;display:grid;"
    "grid-template-columns:52px minmax(0,1fr) 28px;align-items:center;gap:14px;padding:18px 17px 18px 20px;"
    "border:1px solid #d5dfe8;border-radius:14px;background:#fff;color:inherit;text-align:left;"
    "box-shadow:0 5px 15px rgba(16,42,70,.055),inset 0 1px 0 #fff;"
    "transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease;overflow:hidden}",
    "guided action card",
)
core = replace_one(
    core,
    r"\.guided-action-icon\{[^{}]*\}",
    ".guided-action-icon{width:48px;height:48px;display:grid;place-items:center;"
    "border:1px solid color-mix(in srgb,var(--action-accent) 26%,#d6e0e9);border-radius:13px;"
    "background:color-mix(in srgb,var(--action-accent) 9%,#fff);"
    "color:color-mix(in srgb,var(--action-accent) 78%,#0a315a);"
    "box-shadow:0 4px 10px color-mix(in srgb,var(--action-accent) 10%,transparent);"
    "font-family:\"Segoe UI\",Arial,sans-serif;font-size:19px;font-weight:800}",
    "guided action icon",
)
core = replace_one(
    core,
...
text = text.replace('build: "2026-09-08.10"', 'build: "2026-09-08.11"', 1)
config.write_text(text, encoding="utf-8")

package = Path("package.json")
text = package.read_text(encoding="utf-8").replace('"version": "11.21.0"', '"version": "11.21.1"', 1)
package.write_text(text, encoding="utf-8")

index = Path("index.html")
text = index.read_text(encoding="utf-8").replace("V11.21.0", "V11.21.1").replace("?v=11.21.0", "?v=11.21.1")
...
text = text.replace(
    "crm-suministros-v11-21-0-20260908-10",
    "crm-suministros-v11-21-1-20260908-11",
    1,
)
