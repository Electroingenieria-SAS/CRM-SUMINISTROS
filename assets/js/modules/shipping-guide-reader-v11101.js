...
function patchSave(){
  ...
  api.saveShippingGuide=async(orderId,payload={})=>{
    ...
    if(!carrierInvoiceNumber)throw new Error("Registra la factura de la transportadora.");
    if(!(carrierCost>0))throw new Error("Registra un costo de flete mayor que 0.");
    return originalSaveShippingGuide(orderId,{...payload,trackingNumber:value(dialog,"trackingNumber")||payload.trackingNumber,carrier:value(dialog,"carrier")||payload.carrier,carrierInvoiceNumber,carrierCost,carrierCostCurrency:"COP"});
  };
}
...
function enhanceDialog(modal){
  ...
  body.innerHTML=`
    ...
    <section class="guide-upload-zone-v11101" data-guide-dropzone tabindex="0" role="button" aria-label="Seleccionar soporte de guía"><input name="guideFile" type="file" accept="application/pdf,.pdf,image/*,.csv,text/csv" hidden><div class="guide-upload-mark-v11101">↑</div><div><strong data-guide-file-title>Seleccionar soporte de guía</strong><small data-guide-file-meta>Arrastra aquí o elige PDF, imagen o CSV · máximo según política del CRM</small></div><button type="button" class="btn btn-ghost" data-guide-choose>Elegir archivo</button></section>
    ...
    <div class="guide-reader-grid-v11101">${field("Transportadora","carrier",oldCarrier,"Ej. Coordinadora, TCC, Servientrega",true)}${field("Número de guía","trackingNumber",oldTracking,"Número o código de seguimiento",true)}${field("Factura de la transportadora","carrierInvoiceNumberV11101","","Número de factura o cobro del transportador",true)}${field("Costo del flete (COP)","carrierCostV11101","","0",true,"number",'step="0.01" min="0.01" inputmode="decimal"')}</div>
  ...
  zone?.addEventListener("drop",event=>{event.preventDefault();zone.classList.remove("dragging");const file=event.dataTransfer?.files?.[0];if(!file)return;const dt=new DataTransfer();dt.items.add(file);input.files=dt.files;input.dispatchEvent(new Event("change",{bubbles:true}))});
...
function bindManual(modal){modal.querySelectorAll(".guide-reader-field-v11101 input").forEach(input=>input.addEventListener("input",()=>{input.dataset.manual="1";const hint=modal.querySelector(`[data-guide-source="${input.name}"]`);if(hint){hint.textContent="Editado manualmente";hint.className="manual"}}))}

async function readGuide(modal,input){
  const file=input.files?.[0];if(!file)return;
  const kind=documentKind(file),title=modal.querySelector("[data-guide-file-title]"),meta=modal.querySelector("[data-guide-file-meta]");
  if(title)title.textContent=file.name;if(meta)meta.textContent=`${kind.toUpperCase()} · ${formatBytes(file.size)}`;
  ...
  try{
    ...
    setAuto(modal,"carrier",parsed.carrier);setAuto(modal,"trackingNumber",parsed.trackingNumber);setAuto(modal,"carrierInvoiceNumberV11101",parsed.carrierInvoiceNumber);setAuto(modal,"carrierCostV11101",parsed.carrierCost?String(parsed.carrierCost):"");
    const missing=[];if(!parsed.carrier)missing.push("transportadora");if(!parsed.trackingNumber)missing.push("guía");if(!parsed.carrierInvoiceNumber)missing.push("factura del transportador");if(!(parsed.carrierCost>0))missing.push("costo del flete");
    updateStatus(modal,missing.length?"review":"ready",1,missing.length?"Lectura terminada · completa lo pendiente":"Datos detectados","Revisa y corrige antes de guardar. "+(missing.length?`No se detectó: ${missing.join(", ")}.`:""));
  }catch(error){updateStatus(modal,"review",1,"Completa manualmente",error?.message||"No fue posible leer el archivo automáticamente.")}
}

function parseGuideText(text){const raw=String(text||"").replace(/\r/g,"\n"),lines=raw.split(/\n+/).map(clean).filter(Boolean);return {carrier:detectCarrier(lines,raw),trackingNumber:detectTracking(lines,raw),carrierInvoiceNumber:detectCarrierInvoice(lines,raw),carrierCost:detectFreightCost(lines)}}
function detectCarrier(lines,raw){for(const line of lines){const m=line.match(/(?:TRANSPORTADORA|TRANSPORTADOR|CARRIER|OPERADOR\s+LOG[ÍI]STICO|EMPRESA\s+TRANSPORTADORA)\s*[:=\-]\s*(.+)$/i);if(m?.[1])return clean(m[1]).slice(0,100)}const known=["SERVIENTREGA","COORDINADORA","TCC","INTERRAPIDISIMO","INTER RAPIDISIMO","ENVIA","DEPRISA","DHL","FEDEX","UPS"];const upper=raw.toUpperCase();return known.find(name=>upper.includes(name))||""}
function detectTracking(lines,raw){const patterns=[/(?:N[ÚU]MERO\s+DE\s+GU[IÍ]A|NRO\.?\s*GU[IÍ]A|GU[IÍ]A|TRACKING(?:\s+NUMBER)?|REMESA|AWB)\s*[:#=\-]?\s*([A-Z0-9-]{5,})/i];for(const pattern of patterns){const m=raw.match(pattern);if(m?.[1])return clean(m[1]).replace(/\s/g,"")}for(const line of lines){if(!/gu[ií]a|tracking|remesa|awb/i.test(line))continue;const m=line.match(/\b([A-Z0-9-]{6,})\b/i);if(m?.[1])return m[1]}return ""}
function detectCarrierInvoice(lines,raw){const m=raw.match(/(?:FACTURA(?:\s+(?:TRANSPORTADORA|TRANSPORTADOR|FLETE))?|NRO\.?\s*FACTURA|INVOICE)\s*[:#=\-]?\s*([A-Z0-9.-]{3,})/i);if(m?.[1])return clean(m[1]).replace(/\s/g,"");for(const line of lines){if(!/factura|invoice/i.test(line))continue;const x=line.match(/\b([A-Z]{0,5}[-.]?\d{3,}[A-Z0-9.-]*)\b/i);if(x?.[1])return x[1]}return ""}
function detectFreightCost(lines){for(const line of lines){if(!/(COSTO|VALOR|TOTAL).*(FLETE|TRANSPORTE)|FLETE.*(COSTO|VALOR|TOTAL)|FREIGHT/i.test(line))continue;const nums=line.match(/(?:\$|COP\s*)?[\d][\d.,]*/gi)||[];for(let i=nums.length-1;i>=0;i--){const n=localizedNumber(nums[i]);if(n>0)return n}}return null}
