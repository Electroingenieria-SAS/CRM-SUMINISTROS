...
function userWizard({user=null,roles=[],reload,provision=false}){
  ...
  const passwordStep=creatingAccess?{title:"Contraseña inicial",description:"Define la contraseña con la que la persona ingresará por primera vez.",content:`<div class="form-grid"><div class="field"><label>Contraseña *</label><input class="control" name="password" type="password" minlength="12" autocomplete="new-password" required></div><div class="field"><label>Confirmar contraseña *</label><input class="control" name="passwordConfirm" type="password" minlength="12" autocomplete="new-password" required></div></div><div class="wizard-tip">La contraseña nunca se almacena en el perfil ERP ni se muestra después. Solo Supabase Auth conserva su hash seguro.</div>`,validate:({data})=>{if(data.password!==data.passwordConfirm)throw new Error("Las contraseñas no coinciden.");return true}}:null;
  const steps=[
