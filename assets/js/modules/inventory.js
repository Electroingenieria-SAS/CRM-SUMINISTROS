...
const KEY="crm_inventory_view_v11240";
...
export async function renderInventory(root){
  ...
  else{root.innerHTML=empty("Inventario restringido","Tu perfil no tiene una experiencia de Inventario asignada.");return}
  let current=sessionStorage.getItem(KEY);if(!views.some(([id])=>id===current))current=views[0][0];
  const helper=operator?"Cuenta, mide cable, escanea e imprime etiquetas sin ver saldos.":superAdmin?"Planifica, revisa conteos, controla el exprés, consulta existencias y analiza el inventario.":"Planifica, revisa, consulta existencias y analiza el inventario.";
  root.innerHTML=`<section class="inventory-filter-shell-v11109"><div class="inventory-search-main-v11109"><label>Centro de Inventario</label><small>${helper}</small></div><div class="inventory-stock-tabs-v11109">${views.map(([id,label])=>`<button type="button" data-inventory-view="${id}" class="${id===current?"active":""}">${label}</button>`).join("")}</div></section><div id="inventory-workspace-v11240">${loading()}</div>`;
  const host=root.querySelector("#inventory-workspace-v11240"),buttons=[...root.querySelectorAll("[data-inventory-view]")];
  const open=async view=>{current=views.some(([id])=>id===view)?view:views[0][0];sessionStorage.setItem(KEY,current);buttons.forEach(button=>button.classList.toggle("active",button.dataset.inventoryView===current));host.innerHTML=loading();if(operator)return renderInventoryOperator(host,{view:current==="labels"?"labels":"count"});if(current==="plan")return renderInventoryPlan(host);if(current==="review")return renderInventoryReview(host,{status:"SUBMITTED",scope:"ALL"});if(current==="express")return renderInventoryReview(host,{status:"SUBMITTED",scope:"EXPRESS"});if(current==="stock")return renderInventoryStock(host);return renderInventoryControl(host)};
