...
export async function renderInventory(root){
  ...
  if(operator)views.push(["capture","Registrar conteo"],["count","Mi jornada"],["labels","Etiquetas"]);
  ...
  const open=async view=>{
    current=views.some(([id])=>id===view)?view:"home";sessionStorage.setItem(KEY,current);buttons.forEach(b=>b.classList.toggle("active",b.dataset.inventoryView===current));host.innerHTML=loading();
    if(current==="home")return renderInventoryHome(host,{onNavigate:open});
    if(current==="capture")return renderInventoryOperator(host,{view:"capture"});
    if(current==="count")return renderInventoryOperator(host,{view:"count"});
    ...
    if(current==="express-review")return renderInventoryReview(host,{status:"SUBMITTED",scope:"EXPRESS"});
    if(current==="history")return renderInventoryReview(host,{status:"ALL",scope:"ALL",history:true});
    if(current==="stock")return renderInventoryStock(host);
    if(current==="ledger")return renderInventoryLedger(host);
