/* CRM Suministros · Paco Visual Bridge V11.19.2
   Real-image faces, wider 640px workspace and face-to-lightning open-state morph. */
...
function upgradeFace(node){
  ...
  img.classList.add("paco-face-img-v11192");
...
function panelWidth(root){
  ...
  if(!panel)return PANEL_FALLBACK_WIDTH;
  const rect=panel.getBoundingClientRect();
  const cssWidth=parseFloat(getComputedStyle(panel).width)||0;
  return Math.max(300,Math.round(rect.width||cssWidth||PANEL_FALLBACK_WIDTH));
}
...
function computeRoll(root){
  ...
  root.style.setProperty("--paco-roll-distance-v11192",`${Math.round(distance)}px`);
  root.style.setProperty("--paco-panel-width-v11192",`${Math.round(Math.min(PANEL_FALLBACK_WIDTH,Math.max(300,width)))}px`);
}

function setLauncherAccessibility(root,isOpen){
  const launcher=root.querySelector(".paco-launcher");
  if(!launcher)return;
...
function syncOpenState(root){
  ...
  if(isOpen){
    root.classList.add("paco-opening-v11192");
    setLauncherAccessibility(root,true);
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      root.classList.add("paco-open-v11192");
      root.classList.remove("paco-opening-v11192");
      setLauncherAccessibility(root,true);
    }));
  }else{
    root.classList.remove("paco-open-v11192","paco-opening-v11192");
    setLauncherAccessibility(root,false);
  }
}
...
function wireRoot(root){
  ...
  root.classList.remove(
    "paco-visual-v11183","paco-open-v11183","paco-opening-v11183",
    "paco-visual-v11191","paco-open-v11191","paco-opening-v11191"
  );
  root.classList.add("paco-visual-v11192");
  upgradeFaces(root);
  computeRoll(root);
  syncOpenState(root);
  ...
  launcher?.addEventListener("click",()=>{
    ...
    if(opening){computeRoll(root);root.classList.add("paco-opening-v11192")}
  },{capture:true});
}
