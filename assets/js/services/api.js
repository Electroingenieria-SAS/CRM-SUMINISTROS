...
async function rpc(name,params={}){
  ...
  if(error){
    ...
    console.error(`[ERP RPC] ${name}`,{message:error?.message,code:error?.code,status:error?.status});
    ...
    Object.assign(e,error,{rpc:name,technicalMessage:technical});
...
async function edgeFunction(name,body={}){
  ...
  if(error){
    ...
    console.error(`[ERP EDGE] ${name}`,{message:error?.message,status:error?.status});
    const e=new Error(friendly(technical||error.message));
...
export const api={
  ...
  freightIntelligence:(from=null,to=null)=>rpc("erp_x_freight_intelligence_dashboard",{p_from:from||null,p_to:to||null}),
  freightAlerts:(limit=12)=>rpc("erp_x_freight_alerts",{p_limit:limit}),
