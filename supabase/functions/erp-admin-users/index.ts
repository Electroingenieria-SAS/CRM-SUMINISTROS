...
const configuredOrigins = (Deno.env.get("ERP_ALLOWED_ORIGINS") ?? "")
  ...
  .filter(Boolean);
const developmentOrigins = ["http://127.0.0.1:4173", "http://localhost:4173"];
...
Deno.serve(async (req: Request) => {
  if (req.method === "OPTIONS") {
    const origin = req.headers.get("Origin") ?? "";
    if (origin && !allowedOrigins.has(origin)) return new Response(null, { status: 403, headers: corsHeaders(req) });
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (!name) return json(req, { error: "Nombre requerido" }, 400);
      if (!email || !email.includes("@")) return json(req, { error: "Correo inválido" }, 400);
      if (password.length < 12) return json(req, { error: "La contraseña debe tener al menos 12 caracteres." }, 400);
      if (active && selectedRoles.length === 0) return json(req, { error: "Selecciona al menos un rol para un usuario activo." }, 400);

      const existingProfile = profiles.find((p: any) => normalizeEmail(p.email) === email);
      if (existingProfile?.authUserId) return json(req, { error: "Ese perfil ya tiene una cuenta de acceso vinculada." }, 409);
      ...
      try {
        ...
        return json(req, { success: true, profile: saved?.profile, authUserId: created.user.id });
      } catch (error) {
    ...
    if (action === "update") {
      ...
      if (!current) return json(req, { error: "Perfil no encontrado" }, 404);
      ...
      if (!name || !email) return json(req, { error: "Nombre y correo son requeridos" }, 400);
      if (active && selectedRoles.length === 0) return json(req, { error: "Selecciona al menos un rol para un usuario activo." }, 400);
      ...
      try {
        ...
        return json(req, { success: true, profile: saved?.profile });
      } catch (error) {
    ...
    if (action === "password") {
      ...
      const current = profiles.find((p: any) => p.id === profileId);
      if (!current) return json(req, { error: "Perfil no encontrado" }, 404);
      if (!current.authUserId) return json(req, { error: "Este perfil no tiene cuenta de acceso Auth." }, 409);
      if (password.length < 12) return json(req, { error: "La contraseña debe tener al menos 12 caracteres." }, 400);
    ...
    if (action === "delete") {
      ...
      const current = profiles.find((p: any) => p.id === profileId);
      if (!current) return json(req, { error: "Perfil no encontrado" }, 404);
      if (profileId === actorProfileId || current.authUserId === callerAuthId) return json(req, { error: "No puedes eliminar tu propia cuenta Super Admin." }, 409);
      ...
      return json(req, { success: true, profile: detached?.profile });
    }

    return json(req, { error: "Acción administrativa no reconocida" }, 400);
  } catch (error) {
    console.error("[ERP SUPER ADMIN]", error);
    return json(req, { error: publicError(error) }, 400);
  }
});
