-- CRM Suministros V11.27.0 · Auditoría de rendimiento de Inventario
-- Evita construir search_text para ~2k materiales cuando el usuario no está buscando.
-- Mantiene filtros, facetas, paginación y contrato JSON.
...
begin
  ...
  if erp_supply.inventory_count_is_blind_operator() then
    raise exception 'Vista de existencias restringida durante el conteo ciego' using errcode='42501';
  end if;
  ...
  return (
    ...
    searched as(
      select * from base b
      where case
        when v_search='' then true
        else erp_supply.material_norm(
          coalesce(b.reference,'')||' '||coalesce(b.description,'')||' '||coalesce(b.attributes::text,'')||' '||coalesce(b.barcode,'')
        ) like '%'||v_search||'%'
      end
    ),
    filtered as(
...
revoke execute on function public.erp_x_inventory_filtered(jsonb) from public, anon;
grant execute on function public.erp_x_inventory_filtered(jsonb) to authenticated, service_role;
